...
function reconnect(word,id,ex,ey)
  ...
  if bot.status == BotStatus.online and bot:getWorld().name:upper() ~= word:upper() then 
    bot:warp(word)
    sleep(delaywarp)
  end
  if id then 
...
function breakBlock(id,amount)
  ...
  while bot:getInventory:getItemCount(id) > 0 and bot:getInventory:getItemCount(id+1) <= amount do
    if worldz:getTile(math.floor(locals.posx / 32) - 1,math.floor(locals.posy / 32)).fg ~= 0 or worldz:getTile(math.floor(locals.posx / 32) - 1,math.floor(locals.posy / 32)).bg ~= 0 do 
      bot:hit(math.floor(locals.posx / 32) - 1,math.floor(locals.posy / 32))
      sleep(delayBreak)
      reconnect(worldz,last_ex,last_ey)
    end
...
function meMethodkan()
  ...
  bot:wrenchPlayer(getLocal().netid)
  sleep(1000)
  bot:sendPacket(2,"action|dialog_return\ndialog_name|popup\nnetID|"..worldz.netid.."|\nbuttonClicked|open_personalize_profile")
  sleep(1000)
  bot:sendPacket(2,"action|dialog_return\ndialog_name|personalize_profile\nbuttonClicked|100|params_0\n\ncheckbox_show_achievements|0\ncheckbox_show_total_ach_count|0\ncheckbox_show_account_age|0\ncheckbox_show_homeworld|0")
  sleep(1000)
  bot:sendPacket(2,"action|dialog_return\ndialog_name|personalize_profile_achievement\nbuttonClicked|24|params_0,0")
  sleep(1000)
